...
public class CreateGlobalPracticeCommandHandler : IRequestHandler<CreateGlobalPracticeCommand, GlobalPractice>
{
    ...
    public CreateGlobalPracticeCommandHandler(ISemesterRepository semesterRepository, IGlobalPracticeRepository globalPracticeRepository, ISender sender, IMapper mapper)
    {
    ...
    {
        SemesterEntity? lastSemester = (await _semesterRepository.ListAllAsync()).OrderByDescending(semester => semester.EndDate).FirstOrDefault();
        ...
        List<Domain.Entity.Practice> potentialPractices = await _sender.Send(new SearchPotentialPracticeQuery(new SearchPotentialPracticeRequest() { streamId = command.createRequest.streamId }));
